...
\end{equation}
Above (\ref{th:pr-opposites}) is true for any random variable $\X$.

\begin{align} \label{th:normal-dist-symmetry}
\X \sim &\NormalDist (\mu = 0, \sigma) \\
\implies &\pr{\X < x} = 1 - \pr{\X > -x} \\
\iff &\pr{\X < -x} = 1 - \pr{\X > x}
\end{align}
When $\X$ is a symmetrical distribution around 0, for example a standard normal distribution $\Z$, above (\ref{th:normal-dist-symmetry}) is true.
